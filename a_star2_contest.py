    ...
    for ix in range(xWidth+1):
        ...
        for iy in range(yWidth+1):
            ...
            if d < 1.05 * reso:
                ...
                obsMap[int(ix + xMin - airSpace[0][0] / reso)][int(iy + yMin - airSpace[0][1] / reso)] = True  # move the zero points(need to change if the airspace change)
    return obsMap
...
def calMap(reso, airSpace, gate, wall1):
    xWidth = int((airSpace[1][0] - airSpace[0][0]) / reso)
    yWidth = int((airSpace[1][1] - airSpace[0][1]) / reso)
    gatexy = []
...
def calIndex(node, reso, airSpace):
    yWidth = int((airSpace[1][1] - airSpace[0][1]) / reso)
    return int((node.x) * yWidth + (node.y))
...
def verifyNode(node, obsmap, reso, airSpace):
    ...
    '''
    if node.x > (airSpace[1][0] / reso):
        return False
    elif node.y > (airSpace[1][1] / reso):
        return False
    elif node.x <= (airSpace[0][0] / reso):
        return False
    elif node.y <= (airSpace[0][1] / reso):
        return False

    if obsmap[int(node.x - airSpace[0][0] / reso)][int(node.y - airSpace[0][1] / reso)]:
        return False
...
def aStarPlanning(sx, sy, gx, gy):
    ...
    '''
    # wall [sx, sy, ex, ey, z]
    wall = [[5.0,  3.0, 5.5, 3.00, 0.25], [5.0,  3.0, 5.0, 4.75, 0.50]]
    wall1 = [[0.0,  0.0, 6, 0.00],[0.0, 3, 0.0, 4.00]] # cannot cross
    # gate [x, y, angle]
    gate = [[2.00,  1.50, 0.00],
            [3.50,  3.00, 0.00],
            [5.00,  1.50, 0.00],
    ...
    marker = [[2.00,  1.50, 0.10,  0.0, -90.0,   90.0, 1],
              [3.50,  3.00, 0.10,  0.0, -90.0,   90.0, 2],
              [5.00,  1.50, 0.10,  0.0, -90.0,   90.0, 3],
              [5.50,  2.50, 0.10,  0.0, -90.0,  180.0, 4],
              [3.50,  1.50, 0.10,  0.0, -90.0,  -90.0, 5],
              [2.15,  2.50, 0.10,  0.0, -90.0,  -90.0, 6],
              [2.00,  2.50, 0.10,  0.0, -90.0,  -90.0, 7],
              [1.00,  2.00, 0.10,  0.0, -90.0,    0.0, 8],
              [5.50,  3.50, 0.00, 90.0,   0.0,    0.0, 10],
              [4.50,  2.50, 0.00, 90.0,   0.0,    0.0, 11],
              [2.50,  1.00, 0.00, 90.0,   0.0,    0.0, 12],
              [4.50,  1.00, 0.00, 90.0,   0.0,    0.0, 13],
              [2.50,  3.50, 0.00, 90.0,   0.0,    0.0, 14],
              [0.50,  3.45, 0.00, 90.0,   0.0,    0.0, 15]]

    airSpace = [[0, 0], [6, 5]]
    reso = 0.1  # Resolution = 10cm
    len_x = int((airSpace[1][0] - airSpace[0][0]) / reso)
    len_y = int((airSpace[1][1] - airSpace[0][1]) / reso)
    nstart = Node(round(sx / reso), round(sy / reso), 0.0, -1)
    ngoal = Node(round(gx / reso), round(gy / reso), 0.0, -1)

    obsMap, gatexy = calMap(reso, airSpace, gate, wall1)
    # show map
    for x in range(len_x):
        for y in range(len_y):
            if obsMap[x][y]:
                plt.plot(x * reso, y * reso, ".k")
    plt.plot(sx, sy, "xr")
    plt.plot(gx, gy, "xb")
    plt.grid(True)
    plt.axis("equal")
    ...
    motion = [[ 1,  0, 1],
              ...
              [-1,  0, 1],
              [ 0, -1, 1],
              [-1, -1, 1.5],  # give more cost to the diagnol one
              [-1,  1, 1.5],
              [ 1, -1, 1.5],
              [ 1,  1, 1.5]]
...
x, y, z, yaw = aStarPlanning(1, 0.5, 4.0, 4.0)
# print(x)
# print(y)
#print(z)
# print(yaw)
