...
def calMap(reso, airSpace, start_x, start_y, goal_x, goal_y):
    ...
    #                    bbx -  aax     ccx  -     ddx         bby  -  aay     ccy  -     ddy
    delta = determinant(start_x-goal_x, wall[0][0]-wall[0][2], start_y-goal_y, wall[0][1]-wall[0][3])
    if delta <= 0.000001 and delta >= -0.000001:  # delta = 0, parallel
        z = 0.4
    #                     ccx  -     aax     ccx  -     ddx         ccy  -     aay     ccy  -     ddy
    namenda = determinant(wall[0][0]-goal_x, wall[0][0]-wall[0][2], wall[0][1]-goal_y, wall[0][1]-wall[0][3]) / delta
    if namenda > 1 or namenda < 0:
        z = 0.4
    #                 bbx  -  aax     ccx  -     aax     bby  -  aay     ccy  -     aay
    miu = determinant(start_x-goal_x, wall[0][0]-goal_x, start_y-goal_y, wall[0][1]-goal_y) / delta
    if miu > 1 or miu < 0:
        z = 0.4
...
def aStarPlanning(sx, sy, gx, gy):
    ...
    obsMap, z = calMap(reso, airSpace, sx, sy, gx, gy)
    # show map
...
x, y, yaw, height = aStarPlanning(0.0, 1.5, -2.5, 0.9)
print(x, y,height)
plt.show()
