...
class DroneMovement:
    def __init__(self, argv=sys.argv):
        ...
        self.lstn = tf2_ros.TransformListener(self.tf_buf)
	...
	corrected_path = []
	# Hard code change for 1,2,7,8 as gates
	for i in range(len(self.path)):
		id = i + 1
		if id == 1 or id == 2 or id == 7 or id == 8:
			corrected_path.append(self.path[i])

	self.path = corrected_path
    ...
    def move(self):
        ...
        if not self.tf_buf.can_transform("cf1/odom", "map", target.header.stamp, timeout=rospy.Duration(0.3)):
            print("No transform from map to odom")
