...
img = cv2.imread('images_mine/test3.png')  # RGB space imput image
...
contours = cv2.findContours(
    binaryImg, cv2.RETR_TREE,
    cv2.CHAIN_APPROX_SIMPLE)[1]  # modification to work in OpenCV 3.4
    ...
    if type == 'triangle':
        for file in listdir('templates/triangle/'):
            ...
            resized_image = cv2.resize(
                image, (candidate.shape[0], candidate.shape[1]))
...
def transform_tri(image, vector1, vector2, other_vec):
    ...
    box = np.array(
        [[vector1[0][0], vector1[0][1]], [vector2[0][0], vector2[0][1]],
         [point1[0][0], point1[0][1]], [point2[0][0], point2[0][1]]],
        dtype="float32")  # obtain a box with the 4 points to transform
    dst = np.array(
        [[0, int(round(30 * np.sqrt(3)))],
         [60, int(round(30 * np.sqrt(3)))], [60, 0], [0, 0]],
        dtype="float32"
    )  # the output size of the box, normalized would be 1 for the base and sqrt(3)/2 for height
...
def transform_circle(image, vertexs, flag):
    ...
    dst = np.array([[0, 0], [len_square - 1, 0],
                    [len_square - 1, len_square - 1], [0, len_square - 1]],
                   dtype="float32")
    ...
    warped = cv2.warpPerspective(
        image, M,
        (len_square,
         len_square))  # the result should be a cercle inside a square
...
def detectTri(cnt, triCnt):
    ...
    if corners == 3 and S1 > 100:
        ...
        prop = np.sum(redMask) / np.sum(
            cntMask)  # cal proportion of expected color
        if prop < 0.7:
            flag = 1
            # ---------- draw and save ROI ---------------------- #
        if flag == 0:
            ...
            index = index + 1
            print(type)
            picPoint = np.array([approx[0][0], approx[1][0], approx[2][0], (cx, cy)], dtype="double")
            pos = triLocalize(picPoint)
    return triCnt, flag
...
def detectCir(cnt, cirCnt):
    ...
    if prop > 0.22:
        ...
        for point in cnt:
            posx = (point[0][0] - ex) * math.cos(-angle) - (
                point[0][1] - ey) * math.sin(-angle)
            posy = (point[0][0] - ex) * math.sin(-angle) + (
                point[0][1] - ey) * math.cos(-angle)
        ...
        if flag != 4:
            ...
            fillCnt = []
            fillCnt.append(cnt)
            cv2.drawContours(cntMask, fillCnt, -1, 255,
                             -1)  # fill contour with white
            ...
            if flag != 0:
                ...
                x = int(ell[0][0])
                y = int(ell[0][1])
                cv2.putText(img, 'cir%d' % flag, (x, y),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
                print(type)
                box = np.array(box, dtype="double")
                pos = cirLocalize(box)
    return cirCnt


def triLocalize(image_points):  # position estimation
    # ref https://blog.csdn.net/yuanlulu/article/details/82763170
    focal_length = 231
    center = (320, 240)
    camera_matrix = np.array([[focal_length, 0, center[0]], [0, focal_length, center[1]], [0, 0, 1]], dtype="double")
    model_points = np.array([(-256, -148, 0), (256, -148, 0), (0, 296, 0), (0, 0, 0)], dtype="double")  # edge point of triangle
    dist_coeffs = np.array([0.061687, -0.049761, -0.008166, 0.004284, 0.000000])
    (success, rotation_vector, translation_vector) = cv2.solvePnP(model_points, image_points, camera_matrix, dist_coeffs, flags=cv2.SOLVEPNP_ITERATIVE)
    return translation_vector


def cirLocalize(image_points):  # position estimation
    # ref https://blog.csdn.net/yuanlulu/article/details/82763170
    focal_length = 231
    center = (320, 240)
    camera_matrix = np.array([[focal_length, 0, center[0]], [0, focal_length, center[1]], [0, 0, 1]], dtype="double")
    model_points = np.array([(-256, -256, 0), (-256, 256, 0), (256, 256, 0), (256, -256, 0)], dtype="double")  # edge point of rect out of circle
    dist_coeffs = np.array([0.061687, -0.049761, -0.008166, 0.004284, 0.000000])
    (success, rotation_vector, translation_vector) = cv2.solvePnP(model_points, image_points, camera_matrix, dist_coeffs, flags=cv2.SOLVEPNP_ITERATIVE)
    return translation_vector
...
for cnt in contours:
    triCnt, flag = detectTri(cnt, triCnt)
    if flag == 0:
        continue
